...
class Update_fimware extends CI_Controller {
  ...
  public function __construct() {
    ...
    public function do_upload($id_penguna) {
          ...
          $config['allowed_types']        = 'bin|gif|jpg|png';
          $config['max_size']             = 300;
    ...
    public function publish(){
        ...
        // $host   = "192.168.137.2";
        // $port     = 1883;
        $username = $this->input->post('username_server');
        $password = $this->input->post('password_server');
        $qos = $this->input->post('qos');
        $topic = $this->input->post('topic');
         $message =  file_get_contents('asset/uploads/'.$this->input->post('file_name'));
      //  $mess = $this->input->post('file_name');
        //tes sincron
          ...
          $this->session->set_flashdata('message', 'publish to device IoT Success');
          redirect(site_url('update_fimware'));
    		 }else{
